...
public class CreateLimitQrCodeService extends BaseService{
     ...
     * @throws Exception
     */
    public String request(CreateLimitQrCodeReqDto createLimitQrCodeReqDto) throws BusinessException {
    	String responseString = "";
    	try {
    		responseString = sendPost(createLimitQrCodeReqDto, false).trim();
    	} catch(Exception e) {
    		e.printStackTrace();
	    ...
	    if(null == rsp || StringUtils.isBlank(rsp.getTicket())) {
			//通信失败
			Logger.error("微信创建永久二维码接口通信失败，请求数据为：%s, 返回数据为：%s", JSONObject.fromObject(createLimitQrCodeReqDto).toString(),
					JSONObject.fromObject(rsp).toString());
			 throw new BusinessException("微信创建永久二维码接口通信失败");
		} else {
			Logger.info("创建永久二维码API成功返回数据");
			//向微信请求下载
