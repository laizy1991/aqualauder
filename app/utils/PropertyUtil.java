...
public class PropertyUtil {
	 ...
	 * 复制属性值
	 */
	public static void copyPropertyValue(Object src, Object desc) {
		if(null == src) 
			return;
		
		try {
			if(null == desc) {
				desc = Class.forName(src.getClass().getName()).newInstance();
			}
			Field[] fields = src.getClass().getDeclaredFields();
			for(Field f : fields){
				setProperty(desc, f.getName(), getProperty(src, f.getName()));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
